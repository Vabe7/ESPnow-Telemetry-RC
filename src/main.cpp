...
//=================================================================================================
  ...
  if (set.fc_io == fc_en)  {
    Log.println("ESPNow In");
    LogScreenPrintln("ESPNow In");
  }
...
//=================================================================================================
    ...
    if (esp_wifi_set_channel(WIFI_CHAN, WIFI_SECOND_CHAN_NONE) != ESP_OK) {
      Log.println("Error changing WiFi channel");
      //return;
    }
    Log.println("INIT ESPNow");
    if (esp_now_init() != ESP_OK) {
      Log.println("Error initializing ESP-NOW");
      //return;
    }
    else  {
      Log.println("Initialzed ESP-NOW");
    }
    Log.print("ESP32 MAC Address: ");
    ...
    for (int i = 0; i < PPM_Channels; ++i) {
        rawValues[i] = UINT16_MAX;
    }
    pinMode(PPM_Pin, INPUT);
    attachInterrupt(PPM_Pin, handle_PPM_interrupt, RISING);
  #endif
}
...
void loop() {
 ...
 //====================     H a n d l e   F r S k y   P o r t   T r a f f i c
  ...
  if(mavGood && (millis() - hb_millis) > Heartbeat_Timeout)  {   // if no heartbeat from APM in 15s then assume FC mav not connected
    mavGood=false;
...
void Send_To_FC(uint32_t msg_id) {
  ...
  #if (defined ESP32)
    if (set.fc_io == fc_en)  { // ESPNow to FC
      len = mavlink_msg_to_send_buffer(FCbuf, &G2Fmsg);
      /*if (len < 250) {
        Log.println("Ignored one message because it exceeded the 250 bytes limitation from ESP-Now!");
        return;
      }*/
      esp_err_t result = esp_now_send(broadcastAddress, (uint8_t *) &FCbuf, len);
      #if defined  Debug_FC_Up || defined Debug_GCS_Up || defined Debug_ESP_Now
        if (result == ESP_OK && msg_id) {   //  dont print heartbeat - too much info
          Log.printf("Sent to FC ESP_Now from G2Fmsg: len=%d\n", len);
          PrintMavBuffer(&G2Fmsg);
        }
        else {
          switch (result) {
            case ESP_ERR_ESPNOW_NOT_INIT:
              Log.println("ESP-Now not initialized!");
              break;
            case ESP_ERR_ESPNOW_ARG:
              Log.println("ESP-Now invalid argument!");
              break;
            case ESP_ERR_ESPNOW_IF:
              Log.println("ESP-Now interface error!");
              break;
            default:
              if (!msg_id) {
                Log.println("Uncaught ESP-Now error!");
              }
              break;
          }
        }
...
//=================================================================================================  
  ...
  //===============================       H a n d l e   W i F i   E v e n t s 
   ...
   void RestartWiFiSta() { 
    #if (FC_Mavlink_IO == 4)
      return; //Do not mess with Wifi with ESPNow
    #endif
...
//=================================================================================
  ...
  #endif
  #if (defined ESP32)
    sprintf(temp, "<input type='radio' class='big' name='_fc_io' value='ESP-Now' %s> ESP-Now &nbsp &nbsp", set.fc_io4);
    settingsPage += temp;
  #endif  
...
void OnDataSent(const uint8_t *mac_addr, esp_now_send_status_t status) {
  #if ((defined Debug_ESP_Now) && (defined ESP32) || (defined Debug_FC_UP) && (defined ESP32))
    if (status == ESP_NOW_SEND_SUCCESS) {
      //Log.println("ESPNow send success");
    } 
    else {
      Log.println("ESPNow recepient failed to acknowledge message!");
    }
  #endif
}
  ...
  void promiscuous_rx_cb(void *buf, wifi_promiscuous_pkt_type_t type) {
    ...
    ap_rssi = map((ppkt->rx_ctrl.rssi)*-1,0, 100, 100, 0);
    rssiGood = true;
...
void handle_PPM_interrupt() {
  uint64_t previousMicros = microsAtLastPulse;
  microsAtLastPulse = micros();
  uint64_t time = microsAtLastPulse - previousMicros;
  ...
  if (time > blankTime) {
      ...
      pulseCounter = 0;
      send_RC_Override = true;
  }
  else {
      // Store times between pulses as channel values
      if (pulseCounter < PPM_Channels) {
...
void send_RC_Inputs() {
  send_RC_Override = false;
  if (!mavGood) {return;}
  noInterrupts();
  if(microsAtLastPulse + ppm_pulse_timeout < micros()) {
    Log.println("PPM connection to RC lost!");
    for (int i = 0; i < PPM_Channels; ++i) {
        rawValues[i] = 970; //Trigger failsafe
        //rawValues[i] = UINT16_MAX;
    }
  }
  apo_sysid = Device_sysid;                    // From config.h MP is 255, QGC default is 0
  ...
  apo_targsys = 1;                             // FC
  apo_targcomp = 1; 
  mavlink_msg_rc_channels_override_pack(apo_sysid, apo_compid, &G2Fmsg, ap_targsys, apo_targcomp, 
  rawValues[0], rawValues[1], rawValues[2], rawValues[3], rawValues[4], rawValues[5],
  rawValues[6], rawValues[7], rawValues[8], rawValues[9], rawValues[10], rawValues[11],
  rawValues[12], rawValues[13], rawValues[14], rawValues[15], rawValues[16], rawValues[17]);
  interrupts();
  Send_To_FC(70);
}
